...
class LSTMCell(torch.nn.Module):
        ...
        *, 
        ...
        if not split_gates:
            ...
            for g in ["input", "forget", "cell", "output"]:
                self.igates[g] = torch.nn.Linear(input_dim, hidden_dim, bias=bias, **factory_kwargs)
                self.hgates[g] = torch.nn.Linear(hidden_dim, hidden_dim, bias=bias, **factory_kwargs)
    ...
    def from_float(cls, other, use_precomputed_fake_quant=False, split_gates=False):
        ...
        observed = cls.from_params(
            other.weight_ih, other.weight_hh, other.bias_ih, other.bias_hh, split_gates=split_gates
        )
...
class _LSTMLayer(torch.nn.Module):
    ...
    def from_float(cls, other, layer_idx=0, qconfig=None, **kwargs):
        ...
        layer = cls(input_size, hidden_size, bias, batch_first, bidirectional, split_gates=split_gates)
        ...
        layer.layer_fw = _LSTMSingleLayer.from_params(wi, wh, bi, bh, split_gates=split_gates)
        ...
        if other.bidirectional:
            ...
            layer.layer_bw = _LSTMSingleLayer.from_params(wi, wh, bi, bh, split_gates=split_gates)
        return layer
